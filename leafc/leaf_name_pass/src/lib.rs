...
use leaf_coreapi::diagnose::{CompileTimeErrorKind, DiagCtx, ErrorKind, NamePassErrorKind, MsgKind, LocalizedMessage};
use leaf_coreapi::name_pass::{ArmScopeMap, CatchScopeMap, DoScopeMap, FunScopeMap, NamePassApi, NamePassResult};
use leaf_coreapi::scope::{ScopeId, ScopeKind, ScopePool, SymbolKind};
    ...
    fn resolve_name(
        ...
        if self.scope_pool.lookup(current_scope, name).is_some() {
            ...
            self.diag.emit_error(
                ...
                span,
                LocalizedMessage::new(MsgKind::NamePassUndefinedName, std::iter::once(name.to_string())),
            );
            Err(())
        }
    }
...
impl<'a> NamePassApi<'a> for NamePass<'a> {
    fn new(ast_module: &'a CrateAst, diag: &'a mut DiagCtx) -> Self {
        Self {
            diag,
            ast_module,
    ...
    fn pass(mut self) -> Result<NamePassResult, ()> {
        self.build_scope()?;
        self.resolve()?;
        Ok(NamePassResult {
