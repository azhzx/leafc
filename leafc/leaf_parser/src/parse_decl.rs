...
impl<'a> Parser<'a> {
    ...
    fn parse_generic_params(&mut self)
        -> Result<(Vec<GreenChild<GreenGenericVar>>, usize), ()> {
            ...
            let end_off = if self.current_token().kind == TokenType::Comma {
            ...
            } else {
                {
                    ...
                    self.diag.emit_error(
                        ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidGenericParameterList)),
                        span.clone(),
                        LocalizedMessage::new(MsgKind::ParserInvalidGenericParameterList, ["invalid generic parameter list"]),
                    );
                    return Err(());
                }
            };
                ...
                while self.current_token().kind != TokenType::Rparen {
                    ...
                    if self.current_token().kind == TokenType::Comma {
                    ...
                    } else {
                        {
                            let span = self.current_token().span.clone();
                            self.diag.emit_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidFunctionParameterList)),
                                span.clone(),
                                LocalizedMessage::new(MsgKind::ParserInvalidFunctionParameterList, ["invalid parameter list in effect control"]),
                            );
                            return Err(());
                        }
                    }
                ...
                if self.current_token().kind == TokenType::Comma {
                ...
                } else {
                    {
                        ...
                        self.diag.emit_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidFunctionParameterList)),
                            span.clone(),
                            LocalizedMessage::new(MsgKind::ParserInvalidFunctionParameterList, ["invalid method parameter list"]),
                        );
                        return Err(());
                    }
                }
        ...
        match self.current_token().kind {
            ...
            TokenType::NewLine => {
                ...
                if self.current_token().kind == TokenType::Ident {
                ...
                } else {
                    {
                        let span = self.current_token().span.clone();
                        self.diag.emit_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidTypeDeclaration)),
                            span.clone(),
                            LocalizedMessage::new(MsgKind::ParserInvalidTypeDeclaration, ["invalid type declaration body"]),
                        );
                        Err(())
                    }
                }
            ...
            _ => {
                ...
                self.diag.emit_error(
                    ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidTypeDeclaration)),
                    span.clone(),
                    LocalizedMessage::new(MsgKind::ParserInvalidTypeDeclaration, ["unexpected token after type name"]),
                );
                Err(())
            }
        }
            ...
            if self.current_token().kind == TokenType::Comma {
            ...
            } else {
                {
                    ...
                    self.diag.emit_error(
                        ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidFunctionParameterList)),
                        span.clone(),
                        LocalizedMessage::new(MsgKind::ParserInvalidFunctionParameterList, ["invalid function parameter list"]),
                    );
                    return Err(());
                }
            }
            ...
            if self.current_token().kind == TokenType::Comma {
            ...
            } else {
                {
                    ...
                    self.diag.emit_error(
                        ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidFunctionParameterList)),
                        span.clone(),
                        LocalizedMessage::new(MsgKind::ParserInvalidFunctionParameterList, ["invalid external function parameter list"]),
                    );
                    return Err(());
                }
            }
