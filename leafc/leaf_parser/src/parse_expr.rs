use crate::Parser;
use leaf_coreapi::ast::{AtomExprNode, ExprRedNode, GreenCallArg, GreenCatchClause, GreenCatchParam, GreenChild, GreenElseIf, GreenExpr, GreenExprKind, GreenMatchArm, GreenPattern, GreenPureStaticPath, GreenStructFieldInit, GreenStructPatternField, HasTextLen, IdentName, TypeName};
use leaf_coreapi::crate_meta::OperatorKind;
...
use leaf_coreapi::operators::{token_type_to_operator, Operator};
...
impl<'a> Parser<'a> {
    ...
    /// pattern (with or)
    fn parse_pattern(&mut self) -> Result<GreenPattern, ()> {
    ...
    /// pattern unit
    fn parse_atomic_pattern(&mut self) -> Result<GreenPattern, ()> {
        let start_off = self.current_token().span.start_off ;
            ...
            return match self.current_token().kind {
                ...
                _ => {
                    ...
                    self.diag.emit_error(
                        ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidPattern)),
                        span.clone(),
                        LocalizedMessage::new(MsgKind::ParserInvalidPattern, ["expected '(' for constructor or '{' for struct pattern"]),
                    );
                    return Err(());
                }
            };
...

        {
            ...
            self.diag.emit_error(
                ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidPattern)),
                span.clone(),
                LocalizedMessage::new(MsgKind::ParserInvalidPattern, ["unexpected token in pattern"]),
            );
            return Err(());
        }
    }
    ...
    /// pattern binding x
    fn wrap_alias(&mut self, inner: GreenPattern, alias_start_off: usize, pattern_start_off: usize) -> Result<GreenPattern, ()> {
    ...
    /// Tuple
    fn parse_tuple_pattern(&mut self, start_off: usize) -> Result<GreenPattern, ()> {
        self.skip_token(); // '('
        ...
        while self.current_token().kind != TokenType::Rparen {
            if self.current_token().kind == TokenType::DotDot {
                if has_rest {
                    ...
                    self.diag.emit_error(
                        ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidPattern)),
                        span.clone(),
                        LocalizedMessage::new(MsgKind::ParserInvalidPattern, ["multiple '..' in tuple pattern"]),
                    );
                    return Err(());
                }
            ...
            if self.current_token().kind == TokenType::Comma {
                ...
                self.diag.emit_error(
                    ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidPattern)),
                    span.clone(),
                    LocalizedMessage::new(MsgKind::ParserInvalidPattern, ["expected ',' or ')' in tuple pattern"]),
                );
                return Err(());
            }
        }
        ...
        while self.current_token().kind != TokenType::Rparen {
            if self.current_token().kind == TokenType::DotDot {
                if has_rest {
                    ...
                    self.diag.emit_error(
                        ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidPattern)),
                        span.clone(),
                        LocalizedMessage::new(MsgKind::ParserInvalidPattern, ["multiple '..' in constructor pattern"]),
                    );
                    return Err(());
                }
            ...
            if self.current_token().kind == TokenType::Comma {
                ...
                self.diag.emit_error(
                    ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidPattern)),
                    span.clone(),
                    LocalizedMessage::new(MsgKind::ParserInvalidPattern, ["expected ',' or ')' in constructor pattern"]),
                );
                return Err(());
            }
        }
        ...
        while self.current_token().kind != TokenType::Rbrace {
            if self.current_token().kind == TokenType::DotDot {
                if rest {
                    ...
                    self.diag.emit_error(
                        ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidPattern)),
                        span.clone(),
                        LocalizedMessage::new(MsgKind::ParserInvalidPattern, ["multiple '..' in struct pattern"]),
                    );
                    return Err(());
                }
                self.skip_token(); // '..'
            ...
            if self.current_token().kind == TokenType::Comma {
                ...
                self.diag.emit_error(
                    ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidPattern)),
                    span.clone(),
                    LocalizedMessage::new(MsgKind::ParserInvalidPattern, ["expected ',' or '}' in struct pattern"]),
                );
                return Err(());
            }
        }
    ...
    /// pattern => expr
    fn parse_match_arm(&mut self, match_start: usize) -> Result<GreenMatchArm, ()> {
    ...
    /// when expr
    pub fn parse_match_expr(&mut self) -> Result<ExprRedNode, ()> {
    ...
    /// raise expr
    pub fn parse_raise_expr(&mut self) -> Result<ExprRedNode, ()> {
        ...
        while self.current_token().kind != TokenType::Rparen {
            ...
            if self.current_token().kind == TokenType::Comma {
                ...
                let span = self.current_token().span.clone();
                self.diag.emit_error(
                    ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidCallArgumentList)),
                    span.clone(),
                    LocalizedMessage::new(MsgKind::ParserInvalidCallArgumentList, ["invalid raise argument list"]),
                );
                return Err(());
            }
        }
    ...
    /// AE handler
    pub fn parse_with_expr(&mut self) -> Result<ExprRedNode, ()> {
            ...
            while self.current_token().kind != TokenType::Rparen {
                ...
                if self.current_token().kind == TokenType::DotDot {
                    if has_rest {
                        ...
                        self.diag.emit_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidPattern)),
                            span.clone(),
                            LocalizedMessage::new(MsgKind::ParserInvalidPattern, ["multiple `..` in catch parameters"]),
                        );
                        return Err(());
                    }
                    self.skip_token(); // '..'
                    ...
                    self.diag.emit_error(
                        ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidPattern)),
                        span.clone(),
                        LocalizedMessage::new(MsgKind::ParserInvalidPattern, ["catch parameter must be `binding <name>` or `..`"]),
                    );
                    return Err(());
                }
                ...
                if self.current_token().kind == TokenType::Comma {
                    ...
                    self.diag.emit_error(
                        ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidPattern)),
                        span.clone(),
                        LocalizedMessage::new(MsgKind::ParserInvalidPattern, ["expected `,` or `)` in catch parameters"]),
                    );
                    return Err(());
                }
            }
    ...
    /// resume
    pub fn parse_resume_expr(&mut self) -> Result<ExprRedNode, ()> {
    ...
    pub fn parse_unsafe_call_external_expr(&mut self) -> Result<ExprRedNode, ()> {
        ...
        if self.current_token().kind != TokenType::Lparen {
            ...
            self.diag.emit_error(
                ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidCallArgumentList)),
                span.clone(),
                LocalizedMessage::new(MsgKind::ParserInvalidCallArgumentList, ["expected '(' after callee in unsafe external call"]),
            );
            return Err(());
        }
        self.skip_token(); // '('
        ...
        while self.current_token().kind != TokenType::Rparen {
            ...
            if self.current_token().kind == TokenType::Comma {
                ...
                self.diag.emit_error(
                    ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidCallArgumentList)),
                    span.clone(),
                    LocalizedMessage::new(MsgKind::ParserInvalidCallArgumentList, ["invalid argument list in unsafe external call"]),
                );
                return Err(());
            }
        }
    ...
    /// block
    pub fn parse_block_expr(&mut self) -> Result<ExprRedNode, ()> {
    ...
    /// let expr
    pub fn parse_let_expr(&mut self) -> Result<ExprRedNode, ()> {
    ...
    /// do expr
    pub fn parse_do_expr(&mut self) -> Result<ExprRedNode, ()> {
    ...
    /// if / elif / else expr
    pub fn parse_if_expr(&mut self) -> Result<ExprRedNode, ()> {
    ...
    /// const(expr)
    pub fn parse_const_expr(&mut self) -> Result<ExprRedNode, ()> {
    ...
    /// return expr
    pub fn parse_return_expr(&mut self) -> Result<ExprRedNode, ()> {
    ...
    /// atom expr
    pub fn parse_atom_expr(&mut self) -> Result<AtomExprNode, ()> {
        ...
        match current_token_kind {
            ...
            _ => {
                ...
                self.diag.emit_error(
                    ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidExpression)),
                    span.clone(),
                    LocalizedMessage::new(MsgKind::ParserInvalidExpression, ["invalid expression literal"]),
                );
                return Err(());
            }
        }
    ...
    /// main dispatcher
    pub fn parse_expr(&mut self) -> Result<ExprRedNode, ()> {
    ...
    /// Pratt Core
    fn parse_expr_bp(&mut self, min_bp: usize) -> Result<ExprRedNode, ()> {
                ...
                if let Some((prio, op_kind)) = self.user_op_info.get(&token.text) {
                    if *op_kind == OperatorKind::Prefix {
                        ...
                        self.diag.emit_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidExpression)),
                            span.clone(),
                            LocalizedMessage::new(MsgKind::ParserInvalidExpression, [format!("Unexpected operator '{}' in prefix position", token.text)]),
                        );
                        return Err(());
                    }
                } else {
                    ...
                    match self.current_token().kind {
                        ...
                        _ => {
                            ...
                            self.diag.emit_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidExpression)),
                                span.clone(),
                                LocalizedMessage::new(MsgKind::ParserInvalidExpression, ["expected ')' or ',' after expression in parentheses"]),
                            );
                            return Err(());
                        }
                    }
            ...
            if let Some(lbp) = lbp {
                ...
                match kind {
                        ...
                        match op_kind {
                            ...
                            _ => {
                                ...
                                self.diag.emit_error(
                                    ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidExpression)),
                                    span.clone(),
                                    LocalizedMessage::new(MsgKind::ParserInvalidExpression, [format!("Unexpected operator '{}' in infix/postfix position", token.text)]),
                                );
                                return Err(());
                            }
                        }
                    ...
                    _ => {
                        ...
                        let op_child = GreenChild {
                            relative_start: token_start - expr_start,
                            node: Arc::new(token_type_to_operator(&kind).ok_or_else(|| {
                                let span = token.span.clone();
                                self.diag.emit_error(
                                    ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidOperator)),
                                    span.clone(),
                                    LocalizedMessage::new(MsgKind::ParserInvalidOperator, ["invalid operator"]),
                                );
                                ()
                            })?),
                        };
                    ...
                    while self.current_token().kind != TokenType::Rparen {
                        ...
                        if self.current_token().kind == TokenType::Comma {
                            ...
                            self.diag.emit_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidCallArgumentList)),
                                span.clone(),
                                LocalizedMessage::new(MsgKind::ParserInvalidCallArgumentList, ["expected ',' or ')'"]),
                            );
                            return Err(());
                        }
                    }
                ...
                TokenType::Dot => {
                    ...
                    if self.current_token().kind == TokenType::Int {
                        ...
                        let index: usize = index_token.text.parse().map_err(|_| {
                            let span = index_token.span.clone();
                            self.diag.emit_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidExpression)),
                                span.clone(),
                                LocalizedMessage::new(MsgKind::ParserInvalidExpression, ["invalid tuple index, expected a non-negative integer"]),
                            );
                            ()
                        })?;
                ...
                TokenType::Lbrace => {
                    ...
                    while self.current_token().kind != TokenType::Rbrace {
                        ...
                        if field_name_token.kind != TokenType::Ident {
                            let span = field_name_token.span.clone();
                            self.diag.emit_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidStructInit)),
                                span.clone(),
                                LocalizedMessage::new(MsgKind::ParserInvalidStructInit, ["expected field name"]),
                            );
                            return Err(());
                        }
                        ...
                        if self.current_token().kind == TokenType::Comma {
                            ...
                            self.diag.emit_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::ParserError(ParserErrorKind::InvalidStructInit)),
                                span.clone(),
                                LocalizedMessage::new(MsgKind::ParserInvalidStructInit, ["expected ',' or '}'"]),
                            );
                            return Err(());
                        }
                    }
