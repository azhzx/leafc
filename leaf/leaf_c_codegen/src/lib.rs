...
impl CCodeGen {
    ...
    fn ty_to_mangle(&self, ty_id: TyId) -> String {
        let pool = &self.ty_ctx.type_pool;
        ...
        match &pool[root].kind {
            ...
            | TypeNodeKind::ADT { decl_id, subst, .. } => {
                ...
                if let Some(def) = self.ty_ctx.concrete_type_defs.get(&key) {
                    return def.name.clone();
                }
                if let Some(def) = self.ty_ctx.generic_type_defs.get(decl_id) {
                    ...
                    format!("unknown_decl_{}", decl_id.0)
                }
            }
    ...
    fn place_ty(&self, place: &Place, fun: &MirFun) -> TyId {
        match place {
            Place::Local(id) => fun.local_decls[id.0].ty,
            Place::Static(sid) => self.mono_mir.statics[sid.0].ty,
            ...
            Place::EnumItem { place, variant } => {
                ...
                if let TypeNodeKind::ADT { variants, .. } = &pool[root].kind {
                    variants[variant.0].unwrap_or_else(|| {
                        self.ty_ctx
                            .type_pool
                            .iter()
        ...
        match c {
            ...
            Const::Enum(tag, data) => {
                ...
                if has_payload {
                    ...
                    format!(
                        "({}){{ .tag = {}, .data.v{} = {} }}",
                        type_name, tag.0, tag.0, inner_str
                    )
                } else {
                    format!("({}){{ .tag = {} }}", type_name, tag.0)
                }
            }
    ...
    fn place_to_c(&self, place: &Place, var_names: &HashMap<MirLocalId, String>) -> String {
        match place {
            ...
            Place::EnumItem { place, variant } => {
                ...
                format!("{}.data.v{}", base, variant.0)
            }
        }
    }

    fn static_name(&self, sid: MirStaticId) -> String {
        self.mono_mir.statics[sid.0].name.clone()
    }
    ...
    fn rvalue_to_c_with_ty(
        ...
        match rv {
            ...
            Rvalue::GetFunPtr(fun_id) => {
                let callee = &self.mono_mir.functions[fun_id.0];
    ...
    fn gen_function(&self, fun: &MirFun, fun_id: MirFunId) -> String {
        ...
        let ret_root = get_type_root(&self.ty_ctx.type_pool, ret_ty_id);

        let (ret_str, inner_fun_params, inner_fun_ret) = if let TypeNodeKind::Fun {
            param_tys,
            return_ty,
        } =
            &self.ty_ctx.type_pool[ret_root].kind
        {
        ...
        for &bid in &fun.blocks {
            ...
            } = &blocks[bid.0].terminator
            {
                ...
                while let Some(bid2) = stack.pop() {
                    ...
                    match &blocks[bid2.0].terminator {
                        TerminatorKind::Goto { target, .. } => stack.push(*target),
        ...
        loop {
            ...
            for &bid in &fun.blocks {
                ...
                let term = &blocks[bid.0].terminator;
        ...
        let params: Vec<String> = (0..param_count)
            .map(|i| {
                ...
                let name = var_names[&MirLocalId(local_id)].clone();
                if ty.contains("%s") {
                    ty.replace("%s", &name)
                } else {
                    format!("{} {}", ty, name)
        ...
        // function signature
        if let TypeNodeKind::Fun { .. } = &self.ty_ctx.type_pool[ret_root].kind {
            code += &format!("{} {{\n", ret_str);
        } else {
        ...
        for (i, decl) in fun.local_decls.iter().enumerate() {
            ...
            let name = &var_names[&MirLocalId(i)];
        ...
        for &bid in &fun.blocks {
                ...
                body_info.get(&bid)
            {
                ...
                for (i, &local_id) in args_dest.iter().enumerate() {
                    let ty_c = self.ty_to_c(fun.local_decls[local_id.0].ty);
                    code += &format!(
                        "        {} = ({})_handler_args_{}[{}];\n",
            ...
            for stmt in &block.statements {
                match &stmt.kind {
                    MirStmtKind::Let { local, rvalue } => {
                        let ty = fun.local_decls[local.0].ty;
                        if self.ty_to_c(ty) == "void" {
                            continue;
                        }
                        if is_main && local.0 == 0 {
                            continue;
                        }
                    ...
                    MirStmtKind::Store { place, rvalue } => {
                        ...
                        if is_main && matches!(place, Place::Local(id) if id.0 == 0) {
                            continue;
                        }
    ...
    fn gen_terminator(
        ...
        match &block.terminator {
            TerminatorKind::Goto { target, block_args } => {
                let target_params = &blocks[target.0].block_params;
                for (param_id, arg) in target_params.iter().zip(block_args.iter()) {
                    let param_ty = fun.local_decls[param_id.0].ty;
            ...
            TerminatorKind::Call {
                ...
                let callee = &self.mono_mir.functions[func.0];
    ...
    fn extract_with_body(
        ...
        while let Some(bid) = stack.pop() {
            ...
            match &blocks[bid.0].terminator {
                ...
                TerminatorKind::Call { target, .. } | TerminatorKind::CallByPtr { target, .. } => {
                    if let Some(t) = target {
                        stack.push(MirBasicBlockId(t.0));
                    }
                }
                TerminatorKind::Resume { target, .. } => stack.push(MirBasicBlockId(target.0)),
                _ => {}
            }
        }
            ...
            TerminatorKind::Goto { target, block_args } => {
                if *target == merge_block {
                    let resume_val = if block_args.len() == 1 {
                        ...
                        let target_ty = blocks[target.0]
                            .block_params
                            .first()
                            .map(|id| fun.local_decls[id.0].ty);

                        match arg {
                            Rvalue::Move(Place::Local(id)) | Rvalue::Copy(Place::Local(id)) => {
                                let ty = fun.local_decls[id.0].ty;
                                if self.ty_to_c(ty) == "void" {
                                    "0".to_string()
                                } else {
