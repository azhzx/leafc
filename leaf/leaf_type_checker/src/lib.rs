...
use leaf_coreapi::diagnose::{CompileTimeErrorKind, DiagCollector, DiagCtx, ErrorKind, LocalizedMessage, MsgKind, TypeCheckerErrorKind};
use leaf_coreapi::hir::{HirBinOp, HirCatchParam, HirCrate, HirCtorDef, HirDeclKind, HirExprKind, HirFieldDef, HirGenericParam, HirLit, HirName, HirParam, HirPattern, HirTypeName, HirUnaryOp};
use leaf_coreapi::lang_items::BuiltinType;
use leaf_coreapi::id::{HirDeclId, HirExprId, SymId};
use leaf_coreapi::source::Span;
use leaf_coreapi::type_ctx::{FieldDef, GenericParamDef, HirDeclTypeMap, HirExprTypeMap, LocalBindingTypeMap, NameTypeSchemeMap, TyId, TypeCtx, TypeDef, TypeDefKind, TypeNode, TypeNodeKind, TypeScheme, VariantDef};
...
pub struct TypeChecker {
    hir_crate: HirCrate,
    name_pass_result: ResolvedNames,
...
impl TypeChecker {
    ...
    /// resolve type name
    fn resolve_type_name(&mut self, name: &HirTypeName, span: Span) -> Result<TyId, ()> {
        match name {
            HirTypeName::Named { path, generics } => {
                if let Some(scheme) = self.name_type_map.get(&path.sym_id).cloned() {
                    if !generics.is_empty() {
                        self.diag_collector.borrow_mut().add_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::GenericArityMismatch)),
                            span.clone(),
                            LocalizedMessage::new(MsgKind::TypeCheckerGenericArityMismatch, std::iter::empty::<&str>()),
                        );
                        return Err(());
                    }
                ...
                let decl_id = *self.sym_to_decl.get(&path.sym_id)
                    .ok_or_else(|| {
                        self.diag_collector.borrow_mut().add_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::UndefinedType(path.name.clone()))),
                            span.clone(),
                            LocalizedMessage::new(MsgKind::TypeCheckerUndefinedType, std::iter::once(path.name.clone())),
                        );
                    })?;
                ...
                let scheme = self.decl_type_map.get(&decl_id)
                    .ok_or_else(|| {
                        self.diag_collector.borrow_mut().add_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::TypeNotChecked)),
                            span.clone(),
                            LocalizedMessage::new(MsgKind::TypeCheckerTypeNotChecked, std::iter::empty::<&str>()),
                        );
                    })?.clone();
    ...
    fn copy_type_with_subst(&mut self, ty: TyId, subst: &HashMap<TyId, TyId>) -> Result<TyId, ()> {
        ...
        match self.type_pool[root].kind.clone() {
            ...
            TypeNodeKind::ADT { decl_id, subst: existing_subst, variants } => {
                let new_subst: Result<Vec<_>, _> = existing_subst.iter()
                    .map(|&s| self.copy_type_with_subst(s, subst))
                    .collect();
                let new_variants: Result<Vec<Option<TyId>>, _> = variants.iter()
                    .map(|opt| match opt {
                        Some(ty) => Ok(Some(self.copy_type_with_subst(*ty, subst)?)),
                        None => Ok(None),
                    })
                    .collect();
    ...
    fn check_rigid_escape(&mut self, ty: TyId, level: u32, span: &Span) -> Result<(), ()> {
        ...
        match &kind {
            TypeNodeKind::RigidVar => {
                if self.type_pool[root].level > level {
                    {
                        self.diag_collector.borrow_mut().add_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::TypeMismatch {
                                expected: String::new(),
                                found: "existential type variable escapes its scope".to_string(),
                            })),
                            span.clone(),
                            LocalizedMessage::new(MsgKind::TypeCheckerTypeMismatch, ["", "existential type variable escapes its scope"]),
                        );
                        return Err(());
                    };
                }
    ...
    fn unify(&mut self, t1: TyId, t2: TyId, span: Span) -> Result<(), ()> {
        ...
        match (&k1, &k2) {
                ...
                if r1 == r2 {
                    ...
                    Err({
                        self.diag_collector.borrow_mut().add_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::TypeMismatch {
                                expected: String::new(),
                                found: "cannot unify different existential types".to_string(),
                            })),
                            span,
                            LocalizedMessage::new(MsgKind::TypeCheckerTypeMismatch, ["", "cannot unify different existential types"]),
                        );
                    })
                }
            }
            ...
            (TypeNodeKind::RigidVar, _) | (_, TypeNodeKind::RigidVar) => {
                Err({
                    self.diag_collector.borrow_mut().add_error(
                        ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::TypeMismatch {
                            expected: String::new(),
                            found: "existential type cannot be unified with a concrete type".to_string(),
                        })),
                        span,
                        LocalizedMessage::new(MsgKind::TypeCheckerTypeMismatch, ["", "existential type cannot be unified with a concrete type"]),
                    );
                })
            }
            ...
            (TypeNodeKind::Fun { param_tys: p1, return_ty: r1 },
                ...
                if p1.len() != p2.len() {
                    {
                        self.diag_collector.borrow_mut().add_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::ArityMismatch)),
                            span,
                            LocalizedMessage::new(MsgKind::TypeCheckerArityMismatch, std::iter::empty::<&str>()),
                        );
                        return Err(());
                    };
                }
                    ...
                    if let Err(_) = self.unify(a, b, span.clone()) {
                        {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::TypeMismatch {
                                    expected: self.ty_to_string(t2),
                                    found: self.ty_to_string(t1),
                                })),
                                span,
                                LocalizedMessage::new(MsgKind::TypeCheckerTypeMismatch, [self.ty_to_string(t2), self.ty_to_string(t1)]),
                            );
                            return Err(());
                        };
                    }
            ...
            _ => Err({
                self.diag_collector.borrow_mut().add_error(
                    ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::TypeMismatch {
                        expected: self.ty_to_string(r2),
                        found: self.ty_to_string(r1),
                    })),
                    span,
                    LocalizedMessage::new(MsgKind::TypeCheckerTypeMismatch, [self.ty_to_string(r2), self.ty_to_string(r1)]),
                );
            }),
        }
    }

    fn check_occurs(&mut self, var: TyId, ty: TyId, span: Span) -> Result<(), ()> {
        let root = self.representative(ty);
        if root == var {
            {
                self.diag_collector.borrow_mut().add_error(
                    ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::InfiniteType)),
                    span,
                    LocalizedMessage::new(MsgKind::TypeCheckerInfiniteType, std::iter::empty::<&str>()),
                );
                return Err(());
            };
        }
    ...
    fn check_generic_constraints(
        ...
        let generic_params = match &self.hir_crate.hir_decl_pool[decl_id.0].kind {
        ...
        if quantified.len() != generic_params.len() {
            {
                self.diag_collector.borrow_mut().add_error(
                    ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::InternalError("mismatched quantified vars and generic params".to_string()))),
                    span.clone(),
                    LocalizedMessage::new(MsgKind::TypeCheckerInternalError, ["mismatched quantified vars and generic params"]),
                );
                return Err(());
            };
        }
...

                    {
                        self.diag_collector.borrow_mut().add_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::TypeMismatch {
                                ...
                                found: self.ty_to_string(actual_ty),
                            })),
                            span.clone(),
                            LocalizedMessage::new(MsgKind::TypeCheckerTypeMismatch, [expected_str, self.ty_to_string(actual_ty)]),
                        );
                        return Err(());
                    };
                }
    ...
    fn check_match_exhaustiveness(
        ...
        for (idx, pat) in patterns.iter().enumerate() {
            ...
            if !self.is_useful(&matrix, &q) {
                {
                    self.diag_collector.borrow_mut().add_error(
                        ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::UnreachablePattern)),
                        span.clone(),
                        LocalizedMessage::new(MsgKind::TypeCheckerUnreachablePattern, std::iter::empty::<&str>()),
                    );
                    return Err(());
                };
            }
        ...
        if self.is_useful(&matrix, &q_wildcard) {
            {
                self.diag_collector.borrow_mut().add_error(
                    ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::NonExhaustiveMatch)),
                    span.clone(),
                    LocalizedMessage::new(MsgKind::TypeCheckerNonExhaustiveMatch, std::iter::empty::<&str>()),
                );
                return Err(());
            };
        }
        ...
        match pattern {
            ...
            HirPattern::Constructor { type_name, args, span: pat_span } => {
                    ...
                    _ => {
                        {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::TypeMismatch {
                                    expected: String::new(),
                                    found: "constructor pattern requires a named type".to_string(),
                                })),
                                pat_span.clone(),
                                LocalizedMessage::new(MsgKind::TypeCheckerTypeMismatch, ["", "constructor pattern requires a named type"]),
                            );
                            return Err(());
                        };
                    }
                ...
                let scheme = self
                    ...
                    .ok_or_else(|| {
                        self.diag_collector.borrow_mut().add_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::UndefinedVariable(ctor_name.name.clone()))),
                            self.hir_name_span(ctor_name, pat_span.clone()),
                            LocalizedMessage::new(MsgKind::TypeCheckerUndefinedVariable, std::iter::once(ctor_name.name.clone())),
                        );
                    })?;

                let adt_decl_id = self.get_adt_decl_id_for_constructor(ctor_name.sym_id)
                    .unwrap();
                let adt_gen_count = match &self.hir_crate.hir_decl_pool[adt_decl_id.0].kind {
                    ...
                    _ => {
                        {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::TypeMismatch {
                                    expected: String::new(),
                                    found: format!("`{}` is not a constructor", ctor_name.name),
                                })),
                                pat_span.clone(),
                                LocalizedMessage::new(MsgKind::TypeCheckerTypeMismatch, ["".to_string(), format!("`{}` is not a constructor", ctor_name.name)]),
                            );
                            return Err(());
                        };
                    }
                ...
                if let TypeNodeKind::ADT { decl_id, subst, .. } = &self.type_pool[adt_root].kind.clone() {
                    let adt_scheme = self.decl_type_map.get(decl_id)
                        .ok_or_else(|| {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::InternalError("ADT type scheme not found".to_string()))),
                                pat_span.clone(),
                                LocalizedMessage::new(MsgKind::TypeCheckerInternalError, ["ADT type scheme not found"]),
                            );
                        })?
                        .clone();
                    let mut subst_map = HashMap::new();
            ...
            HirPattern::Struct { path, fields, rest: _, span: pat_span } => {
                let decl_id = *self.sym_to_decl.get(&path.sym_id).ok_or_else(|| {
                    self.diag_collector.borrow_mut().add_error(
                        ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::UndefinedType(path.name.clone()))),
                        pat_span.clone(),
                        LocalizedMessage::new(MsgKind::TypeCheckerUndefinedType, std::iter::once(path.name.clone())),
                ...
                let scheme = self.decl_type_map.get(&decl_id).ok_or_else(|| {
                    self.diag_collector.borrow_mut().add_error(
                        ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::TypeNotChecked)),
                        pat_span.clone(),
                        LocalizedMessage::new(MsgKind::TypeCheckerTypeNotChecked, std::iter::empty::<&str>()),
                ...
                let decl = self.hir_crate.hir_decl_pool[decl_id.0].clone();
                let field_defs = match &decl.kind {
                    ...
                    _ => {
                        self.diag_collector.borrow_mut().add_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::TypeMismatch {
                                expected: String::new(),
                                found: format!("`{}` is not a struct", path.name),
                            })),
                            pat_span.clone(),
                            LocalizedMessage::new(MsgKind::TypeCheckerTypeMismatch, ["".to_string(), format!("`{}` is not a struct", path.name)]),
                        );
                        return Err(());
                    },
                };
                ...
                for field_pat in fields {
                    let def = field_defs.iter().find(|f| f.name.name == field_pat.field_name.name)
                        .ok_or_else(|| {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::FieldNotFound(field_pat.field_name.name.clone()))),
                                field_pat.span.clone(),
                                LocalizedMessage::new(MsgKind::TypeCheckerFieldNotFound, std::iter::once(field_pat.field_name.name.clone())),
                            );
                        })?;
    ...
    /// 特化矩阵
    fn specialize<'b>(&self, p: &[Vec<&'b HirPattern>], ctor: &HirPattern) -> Vec<Vec<&'b HirPattern>> {
        let mut s_p = Vec::new();
        for row in p {
    ...
    fn default_matrix<'b>(&self, p: &[Vec<&'b HirPattern>]) -> Vec<Vec<&'b HirPattern>> {
        let mut d_p = Vec::new();
        for row in p {
            if row.is_empty() { continue; }
            match row[0] {
    ...
    fn clone_type_with_fresh_vars(&mut self, ty: TyId) -> Result<(TyId, HashMap<TyId, TyId>), ()> {
        let mut var_map = HashMap::new();
        let new_ty = self.clone_type_internal(ty, &mut var_map)?;
        Ok((new_ty, var_map))
    }

    fn clone_type_internal(&mut self, ty: TyId, var_map: &mut HashMap<TyId, TyId>) -> Result<TyId, ()> {
        let root = self.representative(ty);
        let kind = self.type_pool[root].kind.clone();
        match kind {
            TypeNodeKind::Var => {
    ...
    fn get_adt_type_param(&mut self, ty: TyId) -> Result<Option<TyId>, ()> {
        let root = self.representative(ty);
    ...
    fn get_constructors<'b>(&self, p: &[Vec<&'b HirPattern>]) -> Vec<&'b HirPattern> {
        let mut ctors: Vec<&'b HirPattern> = Vec::new();
            ...
            HirPattern::Constructor { type_name, .. } => {
                if let HirTypeName::Named { path, .. } = type_name {
                    if let Some(decl_id) = self.get_adt_decl_id_for_constructor(path.sym_id) {
                        let decl = &self.hir_crate.hir_decl_pool[decl_id.0];
    ...
    fn infer_expr(&mut self, expr_id: HirExprId, expected: Option<TyId>) -> Result<TyId, ()> {
        let expr = self.hir_crate.hir_expr_pool[expr_id.0].clone();
        ...
        let ty = match &expr.kind {
            ...
            HirExprKind::Ident(name) => {
                let ty = if let Some(scheme) = self.name_type_map.get(&name.sym_id).cloned() {
                    ...
                    let decl_id = match self.sym_to_decl.get(&name.sym_id) {
                        Some(&id) => id,
                        None => {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::UndefinedVariable(name.name.clone()))),
                                self.hir_name_span(&name, span.clone()),
                                LocalizedMessage::new(MsgKind::TypeCheckerUndefinedVariable, std::iter::once(name.name.clone())),
                            );
                            return Err(());
                        }
                    };
                    let scheme = match self.decl_type_map.get(&decl_id) {
                        Some(s) => s.clone(),
                        None => {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::TypeNotChecked)),
                        ...
                        _ => {
                            {
                                self.diag_collector.borrow_mut().add_error(
                                    ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::TypeMismatch {
                                        expected: String::new(),
                                        found: format!("cannot access field `{}` on non‑struct type", field),
                                    })),
                                    span.clone(),
                                    LocalizedMessage::new(MsgKind::TypeCheckerTypeMismatch, ["".to_string(), format!("cannot access field `{}` on non‑struct type", field)]),
                                );
                                return Err(());
                            };
                        }
                    ...
                    match &decl.kind {
                        HirDeclKind::Struct { fields, generic_params, .. } => {
                            let field_def = fields
                                ...
                                .ok_or_else(|| {
                                    self.diag_collector.borrow_mut().add_error(
                                        ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::FieldNotFound(field.clone()))),
                                        span.clone(),
                                        LocalizedMessage::new(MsgKind::TypeCheckerFieldNotFound, std::iter::once(field.clone())),
                                    );
                                })?;
                        ...
                        _ => {
                            {
                                self.diag_collector.borrow_mut().add_error(
                                    ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::TypeMismatch {
                                        expected: String::new(),
                                        found: "type is not a struct".to_string(),
                                    })),
                                    span.clone(),
                                    LocalizedMessage::new(MsgKind::TypeCheckerTypeMismatch, ["", "type is not a struct"]),
                                );
                                return Err(());
                            };
                        }
                        ...
                        if *index >= actual_elem_tys.len() {
                            {
                                self.diag_collector.borrow_mut().add_error(
                                    ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::TypeMismatch {
                                        expected: String::new(),
                                        found: format!("tuple index {} out of bounds (length {})", index, actual_elem_tys.len()),
                                    })),
                                    span.clone(),
                                    LocalizedMessage::new(MsgKind::TypeCheckerTypeMismatch, ["".to_string(), format!("tuple index {} out of bounds (length {})", index, actual_elem_tys.len())]),
                                );
                                return Err(());
                            };
                        }
                    ...
                    TypeNodeKind::Var => {
                        {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::TypeMismatch {
                                    expected: String::new(),
                                    found: "cannot access tuple index on an unknown type; add a type annotation".to_string(),
                                })),
                                span.clone(),
                                LocalizedMessage::new(MsgKind::TypeCheckerTypeMismatch, ["", "cannot access tuple index on an unknown type; add a type annotation"]),
                            );
                            return Err(());
                        };
                    }
                    _ => {
                        {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::TypeMismatch {
                                    ...
                                    found: self.ty_to_string(expr_ty),
                                })),
                                span.clone(),
                                LocalizedMessage::new(MsgKind::TypeCheckerTypeMismatch, ["a tuple type".to_string(), self.ty_to_string(expr_ty)]),
                            );
                            return Err(());
                        };
                    }
            ...
            HirExprKind::MakeStruct { path, fields } => {
                ...
                } else {
                    {
                        self.diag_collector.borrow_mut().add_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::TypeMismatch {
                                ...
                                found: self.ty_to_string(struct_ty),
                            })),
                            span.clone(),
                            LocalizedMessage::new(MsgKind::TypeCheckerTypeMismatch, ["struct".to_string(), self.ty_to_string(struct_ty)]),
                        );
                        return Err(());
                    };
                };
                ...
                let (generic_params, struct_fields) = match &decl.kind {
                    ...
                    _ => {
                        self.diag_collector.borrow_mut().add_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::InternalError("struct decl_id points to non‑struct".to_string()))),
                            span.clone(),
                            LocalizedMessage::new(MsgKind::TypeCheckerInternalError, ["struct decl_id points to non‑struct"]),
                        );
                        return Err(());
                    },
                };
                ...
                let struct_scheme = self.decl_type_map.get(&decl_id)
                    .ok_or_else(|| {
                        self.diag_collector.borrow_mut().add_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::InternalError("struct type scheme not found".to_string()))),
                            span.clone(),
                            LocalizedMessage::new(MsgKind::TypeCheckerInternalError, ["struct type scheme not found"]),
                        );
                    })?
                    .clone();
                ...
                for (field_name, field_expr) in fields {
                    let def = struct_fields.iter()
                        ...
                        .ok_or_else(|| {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::UnknownField(field_name.clone()))),
                                span.clone(),
                                LocalizedMessage::new(MsgKind::TypeCheckerUnknownField, std::iter::once(field_name.clone())),
                            );
                        })?;
            ...
            HirExprKind::BuildVariant { variant_name, target } => {
                let scheme = match self.name_type_map.get(&variant_name.sym_id) {
                    Some(s) => s.clone(),
                    None => {
                        self.diag_collector.borrow_mut().add_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::UndefinedVariable(variant_name.name.clone()))),
                            self.hir_name_span(variant_name, span.clone()),
                            LocalizedMessage::new(MsgKind::TypeCheckerUndefinedVariable, std::iter::once(variant_name.name.clone())),
                        );
                        return Err(());
                    }
                };
                    ...
                    _ => {
                        {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::TypeMismatch {
                                    expected: String::new(),
                                    found: format!("constructor `{}` has unexpected type", variant_name.name),
                                })),
                                span.clone(),
                                LocalizedMessage::new(MsgKind::TypeCheckerTypeMismatch, ["".to_string(), format!("constructor `{}` has unexpected type", variant_name.name)]),
                            );
                            return Err(());
                        };
                    }
            ...
            HirExprKind::Raise { control_name, args } => {
                let scheme = match self.name_type_map.get(&control_name.sym_id) {
                    Some(s) => s.clone(),
                    None => {
                        self.diag_collector.borrow_mut().add_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::UndefinedVariable(control_name.name.clone()))),
                            self.hir_name_span(control_name, span.clone()),
                            LocalizedMessage::new(MsgKind::TypeCheckerUndefinedVariable, std::iter::once(control_name.name.clone())),
                        );
                        return Err(());
                    }
                };
                ...
                let (param_tys, ret_ty) = if let TypeNodeKind::Fun { param_tys, return_ty } = &self.type_pool[root].kind {
                ...
                } else {
                    {
                        self.diag_collector.borrow_mut().add_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::TypeMismatch {
                                expected: String::new(),
                                found: format!("control `{}` is not a function type", control_name.name),
                            })),
                            span.clone(),
                            LocalizedMessage::new(MsgKind::TypeCheckerTypeMismatch, ["".to_string(), format!("control `{}` is not a function type", control_name.name)]),
                        );
                        return Err(());
                    };
                };
            ...
            HirExprKind::Resume { expr } => {
                let resume_fun_ty = {
                    let (ty, called) = self.current_resume_ty.as_mut()
                        .ok_or_else(|| {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::InternalError("`resume` used outside of a catch clause".to_string()))),
                                span.clone(),
                                LocalizedMessage::new(MsgKind::TypeCheckerInternalError, ["`resume` used outside of a catch clause"]),
                            );
                        })?;
                    ...
                    if *called {
                        {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::MultipleResume)),
                                span.clone(),
                                LocalizedMessage::new(MsgKind::TypeCheckerMultipleResume, std::iter::empty::<&str>()),
                            );
                            return Err(());
                        };
                    }
            ...
            HirExprKind::With { handler, clauses } => {
                ...
                for clause in clauses {
                    let scheme = self.name_type_map.get(&clause.control_path.sym_id)
                        ...
                        .ok_or_else(|| {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::UndefinedVariable(clause.control_path.name.clone()))),
                                clause.span.clone(),
                                LocalizedMessage::new(MsgKind::TypeCheckerUndefinedVariable, std::iter::once(clause.control_path.name.clone())),
                            );
                        })?;
                    ...
                    let (ctrl_param_tys, ctrl_ret_ty) =
                            ...
                            &self.type_pool[ctrl_root].kind
                        {
                            ...
                            {
                                self.diag_collector.borrow_mut().add_error(
                                    ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::InvalidControlType)),
                                    clause.span.clone(),
                                    LocalizedMessage::new(MsgKind::TypeCheckerInvalidControlType, std::iter::empty::<&str>()),
                                );
                                return Err(());
                            };
                        };
                    ...
                    if !is_unit && !resume_called {
                        {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::MissingResume)),
                                clause.span.clone(),
                                LocalizedMessage::new(MsgKind::TypeCheckerMissingResume, std::iter::empty::<&str>()),
                            );
                            return Err(());
                        };
                    }
    ...
    fn infer_lit(&mut self, lit: &HirLit) -> Result<TyId, ()> {
        match lit {
    ...
    fn infer_binary(&mut self, left: HirExprId, right: HirExprId, op: HirBinOp, span: &Span) -> Result<TyId, ()> {
    ...
    fn infer_unary(&mut self, op: HirUnaryOp, right: HirExprId, span: &Span) -> Result<TyId, ()> {
        let rt = self.infer_expr(right, None)?;
        match op {
    ...
    fn infer_call(
        ...
        let callee_kind = self.hir_crate.hir_expr_pool[callee.0].kind.clone();
        ...
        let (mut callee_ty, fun_info) = if let HirExprKind::Ident(ref name) = callee_kind {
            ...
            if let Some((scheme, decl_id_opt)) = scheme_and_decl {
                ...
                let is_fun = decl_id_opt.map_or(false, |id| {
                    matches!(&self.hir_crate.hir_decl_pool[id.0].kind, HirDeclKind::Fun { .. })
                });
                if is_fun {
    ...
    fn infer_block(&mut self, stmts: &[HirExprId], expected: Option<TyId>, _span: &Span) -> Result<TyId, ()> {
        if stmts.is_empty() {
    ...
    fn infer_tuple(&mut self, elements: &[HirExprId], expected: Option<TyId>, _span: &Span) -> Result<TyId, ()> {
    ...
    fn infer_return(&mut self, expr: Option<&HirExprId>, _expected: Option<TyId>, span: &Span) -> Result<TyId, ()> {
        let ret_ty = self.current_expected_return_ty
            .ok_or_else(|| {
                self.diag_collector.borrow_mut().add_error(
                    ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::ReturnOutsideFunction)),
                    span.clone(),
                    LocalizedMessage::new(MsgKind::TypeCheckerReturnOutsideFunction, std::iter::empty::<&str>()),
                );
            })?;
    ...
    fn infer_cast(&mut self, expr: HirExprId, type_ann: &HirTypeName, span: &Span) -> Result<TyId, ()> {
        let target_ty = self.resolve_type_name(type_ann, span.clone())?;
        self.infer_expr(expr, None)?;
        Ok(target_ty)
    }

    fn check_decl(&mut self, decl_id: HirDeclId) -> Result<(), ()> {
        let decl = self.hir_crate.hir_decl_pool[decl_id.0].clone();
        match &decl.kind {
            HirDeclKind::Fun {
                generic_params,
    ...
    fn check_fun(&mut self, decl_id: HirDeclId, generic_params: &[HirGenericParam], params: &[HirParam], return_type: Option<&HirTypeName>, body: &[HirExprId]) -> Result<(), ()> {
        let saved_level = self.current_level;
        ...
        let ret_ty = if let Some(rt) = return_type {
            self.resolve_type_name(rt, self.hir_crate.hir_decl_pool[decl_id.0].span.clone())?
        } else {
            self.new_type_var()
        };
        ...
        self.unify(body_ty, ret_ty, self.hir_crate.hir_decl_pool[decl_id.0].span.clone())?;
        let fun_ty = self.intern_type(TypeNodeKind::Fun {
            param_tys,
            return_ty: ret_ty,
        });
        ...
        if self.hir_crate.exports.contains(&decl_id)
            && (!has_all_param_ty_ann || !has_ret_ty_ann) {
            {
                self.diag_collector.borrow_mut().add_error(
                    ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::MissingTypeAnnotation)),
                    self.hir_crate.hir_decl_pool[decl_id.0].span.clone(),
                    LocalizedMessage::new(MsgKind::TypeCheckerMissingTypeAnnotation, std::iter::empty::<&str>()),
                );
                return Err(());
            };
        }
    ...
    fn check_external(
        ...
        let ret_ty = self.resolve_type_name(return_type, self.hir_crate.hir_decl_pool[decl_id.0].span.clone())?;
    ...
    fn check_type_alias(
        ...
        let self_sym_id = self.sym_to_decl.iter()
            ...
            .ok_or_else(|| {
                self.diag_collector.borrow_mut().add_error(
                    ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::InternalError("self symbol not found".to_string()))),
                    decl.span.clone(),
                    LocalizedMessage::new(MsgKind::TypeCheckerInternalError, ["self symbol not found"]),
                );
            })?;
        ...
        if Self::contains_self_ref(alias_for, self_sym_id) {
            {
                self.diag_collector.borrow_mut().add_error(
                    ErrorKind::CompileTimeError(CompileTimeErrorKind::TypeCheckerError(TypeCheckerErrorKind::RecursiveTypeAlias)),
                    decl.span.clone(),
                    LocalizedMessage::new(MsgKind::TypeCheckerRecursiveTypeAlias, std::iter::empty::<&str>()),
                );
                return Err(());
            };
        }
        ...
        let span = self.hir_crate.hir_decl_pool[decl_id.0].span.clone();
    ...
    fn build_sym_to_decl(&mut self) {
        for (id, decl) in self.hir_crate.hir_decl_pool.iter().enumerate() {
            ...
            {
                if let Some((sym, _)) = self.name_pass_result.pool.lookup(scope_id, &decl.ident) {
                    self.sym_to_decl.insert(sym.sym_id, HirDeclId(id));
                }
            }
        }
    }
    ...
    fn ty_kind_to_string(&self, kind: &TypeNodeKind) -> String {
        match kind {
            ...
            TypeNodeKind::Struct { decl_id, subst, .. } => {
                let ident = self.hir_crate.hir_decl_pool[decl_id.0].ident.clone();
    ...
    fn new(mut hir_crate: HirCrate, mut resolved_names: ResolvedNames) -> Self {
        let name_pass_result = resolved_names;
    ...
    fn check(mut self) -> ((TypeCtx, HirCrate), DiagCollector) {
        ...
        for decl_id in 0..self.hir_crate.hir_decl_pool.len() {
            ...
            self.decl_type_map.insert(HirDeclId(decl_id), TypeScheme { quantified: gen_vars, body });
        }
        for decl_id in 0..self.hir_crate.hir_decl_pool.len() {
            self.check_decl(HirDeclId(decl_id));
        }
        ...
        }, self.hir_crate), self.diag_collector.into_inner())
    }
}