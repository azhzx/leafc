use crate::id::{FileId, HirExprId, SymId};
...
pub enum MsgKind {
    ...
    MirLowerGuardNoPlace,
    MirLowerOrPatternNotExpanded,
    MirLowerExpectedStructType,
    MirLowerFieldMissingInStruct,
    MirLowerExpectedADTType,
    MirLowerVariantMissingInADTMap,
    MirLowerLiteralNoMatchingPattern,
    MirLowerCtorNoMatchingPattern,
    MirLowerIrrefutableColumnNoPattern,
    MirLowerGuardExprNoPlace,
    MirLowerCalleeNoPlace,
    MirLowerExprTypeMissing,
    MirLowerExpectedFnType,
    MirLowerIdentNoValueFound,
    MirLowerBinLeftNoPlace,
    MirLowerBinRightNoPlace,
    MirLowerUnaryOperandNoPlace,
    MirLowerIfCondNoPlace,
    MirLowerElifCondNoPlace,
    MirLowerMatchScrutineeNoPlace,
    MirLowerIsExprNoPlace,
    MirLowerControlNotFound,
    MirLowerControlNotFunctionType,
    MirLowerControlParamIndexOOB,
    MirLowerResumeOutsideHandler,
    MirLowerFieldAccessNonStruct,
    MirLowerMoveSourceNoPlace,
    MirLowerCopySourceNoPlace,
    MirLowerRefTargetNoPlace,
    MirLowerMutRefTargetNoPlace,
    MirLowerShareTargetNoPlace,
    MirLowerCastSourceNoPlace,
    MirLowerVariantBuildNonADT,
    MirLowerGenericInternal,
...
pub enum MirLowerErrorKind {
    #[error("guard expression produced no place")]
    GuardNoPlace,

    #[error("or pattern should have been expanded before match lowering")]
    OrPatternNotExpanded,

    #[error("expected struct type for struct pattern binding")]
    ExpectedStructType,

    #[error("field `{0}` not found in struct definition")]
    FieldMissingInStruct(String),

    #[error("constructor pattern requires ADT/enum type, found other type")]
    ExpectedADTType,

    #[error("variant `{0}` does not exist in ADT variant map")]
    VariantMissingInADTMap(String),

    #[error("no matching pattern branch for literal variant")]
    LiteralNoMatchingPattern,

    #[error("no matching pattern branch for constructor variant `{0:?}`")]
    CtorNoMatchingPattern(SymId),

    #[error("irrefutable column cannot find matching pattern")]
    IrrefutableColumnNoPattern,

    #[error("guard expression evaluated to nothing (no place)")]
    GuardExprNoPlace,

    #[error("call callee expression produces no valid place")]
    CalleeNoPlace,

    #[error("cannot fetch type of expr, typecheck pass missing type info")]
    ExprTypeMissing,

    #[error("type is not a function type, cannot call")]
    ExpectedFnType,

    #[error("cannot resolve value `{0}` for identifier lookup")]
    IdentNoValueFound(String),

    #[error("binary operator left operand has no place")]
    BinLeftNoPlace,

    #[error("binary operator right operand has no place")]
    BinRightNoPlace,

    #[error("unary operator operand has no valid place")]
    UnaryOperandNoPlace,

    #[error("if condition expression returns no place")]
    IfCondNoPlace,

    #[error("elif condition expression returns no place")]
    ElifCondNoPlace,

    #[error("match scrutinee has no place to inspect")]
    MatchScrutineeNoPlace,

    #[error("is() match target expression has no place")]
    IsExprNoPlace,

    #[error("effect control `{0}` not registered in control map")]
    ControlNotFound(String),

    #[error("effect control `{0}` is not a function type")]
    ControlNotFunctionType(String),

    #[error("control {ctrl_name} only has {param_cnt} parameters, index {idx} out of bounds")]
    ControlParamIndexOOB { ctrl_name: String, param_cnt: usize, idx: usize },

    #[error("resume statement used outside effect handler scope")]
    ResumeOutsideHandler,

    #[error("cannot access field `{field}` on non-struct/tuple type")]
    FieldAccessNonStruct { field: String },

    #[error("move expression source has no place")]
    MoveSourceNoPlace,

    #[error("copy expression source has no place")]
    CopySourceNoPlace,

    #[error("ref target expression has no place")]
    RefTargetNoPlace,

    #[error("mut ref target expression has no place")]
    MutRefTargetNoPlace,

    #[error("share(gc ref) target expression has no place")]
    ShareTargetNoPlace,

    #[error("type cast source expression has no place")]
    CastSourceNoPlace,

    #[error("construct enum variant on non-ADT type")]
    VariantBuildNonADT,

    #[error("MIR lower internal error: {0}")]
    GenericInternal(String),
}
...
impl DiagCollector {
    ...
    pub fn push_diag_error(&mut self, err: DiagError) {
        self.errors.push(err);
    }
}
...
pub fn get_message_kind_for_error(error_kind: &ErrorKind) -> Option<(MsgKind, MsgKind)> {
    match error_kind {
        ErrorKind::CompileTimeError(cte) => match cte {
            ...
            CompileTimeErrorKind::MirLowerError(mir_err) => match mir_err {
                MirLowerErrorKind::GuardNoPlace => Some((MsgKind::MirLowerGuardNoPlace, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::OrPatternNotExpanded => Some((MsgKind::MirLowerOrPatternNotExpanded, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::ExpectedStructType => Some((MsgKind::MirLowerExpectedStructType, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::FieldMissingInStruct(_) => Some((MsgKind::MirLowerFieldMissingInStruct, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::ExpectedADTType => Some((MsgKind::MirLowerExpectedADTType, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::VariantMissingInADTMap(_) => Some((MsgKind::MirLowerVariantMissingInADTMap, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::LiteralNoMatchingPattern => Some((MsgKind::MirLowerLiteralNoMatchingPattern, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::CtorNoMatchingPattern(_) => Some((MsgKind::MirLowerCtorNoMatchingPattern, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::IrrefutableColumnNoPattern => Some((MsgKind::MirLowerIrrefutableColumnNoPattern, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::GuardExprNoPlace => Some((MsgKind::MirLowerGuardExprNoPlace, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::CalleeNoPlace => Some((MsgKind::MirLowerCalleeNoPlace, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::ExprTypeMissing => Some((MsgKind::MirLowerExprTypeMissing, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::ExpectedFnType => Some((MsgKind::MirLowerExpectedFnType, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::IdentNoValueFound(_) => Some((MsgKind::MirLowerIdentNoValueFound, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::BinLeftNoPlace => Some((MsgKind::MirLowerBinLeftNoPlace, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::BinRightNoPlace => Some((MsgKind::MirLowerBinRightNoPlace, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::UnaryOperandNoPlace => Some((MsgKind::MirLowerUnaryOperandNoPlace, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::IfCondNoPlace => Some((MsgKind::MirLowerIfCondNoPlace, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::ElifCondNoPlace => Some((MsgKind::MirLowerElifCondNoPlace, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::MatchScrutineeNoPlace => Some((MsgKind::MirLowerMatchScrutineeNoPlace, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::IsExprNoPlace => Some((MsgKind::MirLowerIsExprNoPlace, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::ControlNotFound(_) => Some((MsgKind::MirLowerControlNotFound, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::ControlNotFunctionType(_) => Some((MsgKind::MirLowerControlNotFunctionType, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::ControlParamIndexOOB { .. } => Some((MsgKind::MirLowerControlParamIndexOOB, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::ResumeOutsideHandler => Some((MsgKind::MirLowerResumeOutsideHandler, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::FieldAccessNonStruct { .. } => Some((MsgKind::MirLowerFieldAccessNonStruct, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::MoveSourceNoPlace => Some((MsgKind::MirLowerMoveSourceNoPlace, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::CopySourceNoPlace => Some((MsgKind::MirLowerCopySourceNoPlace, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::RefTargetNoPlace => Some((MsgKind::MirLowerRefTargetNoPlace, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::MutRefTargetNoPlace => Some((MsgKind::MirLowerMutRefTargetNoPlace, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::ShareTargetNoPlace => Some((MsgKind::MirLowerShareTargetNoPlace, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::CastSourceNoPlace => Some((MsgKind::MirLowerCastSourceNoPlace, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::VariantBuildNonADT => Some((MsgKind::MirLowerVariantBuildNonADT, MsgKind::HelpCheckSyntax)),
                MirLowerErrorKind::GenericInternal(_) => Some((MsgKind::MirLowerGenericInternal, MsgKind::HelpCheckSyntax)),
            },
