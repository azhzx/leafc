...
use leaf_coreapi::ast::{child_decl_red, child_expr_red, child_span, child_span_of, AtomExprNode, CrateAst, DeclRedNode, ExprRedNode, FileRedUnit, GreenCatchClause, GreenCatchParam, GreenChild, GreenCtor, GreenDecl, GreenDeclKind, GreenExpr, GreenExprKind, GreenMatchArm, GreenPattern, GreenPureStaticPath, HasTextLen, RequireRedNode, TypeName, Visibility};
use leaf_coreapi::diagnose::{CompileTimeErrorKind, DiagCtx, ErrorKind, NamePassErrorKind, MsgKind, LocalizedMessage, DiagCollector};
use leaf_coreapi::scope::{ArmScopeMap, CatchScopeMap, DoScopeMap, FunScopeMap, ResolvedNames};
use leaf_coreapi::scope::{ScopeKind, ScopePool, SymbolKind};
use leaf_coreapi::source::{Span};
use std::collections::HashMap;
use std::sync::Arc;
use leaf_coreapi::id::{FileId, ScopeId};
use leaf_coreapi::lang_items::{LangItems, STR_TO_BUILTIN};
...
pub struct NamePass<'a> {
    ...
    /// source_id => FileUnit
    source_to_file_unit: HashMap<FileId, &'a FileRedUnit>,

    /// source_id => ScopeId
    source_id_to_scope: HashMap<FileId, ScopeId>,
...
impl<'a> NamePass<'a> {
    ...
    fn collect_pattern_bindings(
        ...
        let pat_span = child_span(
            parent_span, pattern_child.relative_start, pattern.text_len().0
        );
    ...
    fn resolve_static_path_with_span(
        ...
        let first_span = Span {
            ...
            end_off: path_span.start_off + first_seg.relative_start + first_seg.node.text_len().0,
        };

        let (mut sym, mut sym_scope) = match self.scope_pool
            .lookup(current_scope, first_name) {
                Some(pair) => pair,
                None => {
                    self.diag_collector.borrow_mut().add_error(
                        ErrorKind::CompileTimeError(CompileTimeErrorKind::NamePassError(
                            NamePassErrorKind::UndefinedName(first_name.clone()),
                        )),
                        first_span.clone(),
                        LocalizedMessage::new(MsgKind::NamePassUndefinedName, std::iter::once(first_name.clone())),
                    );
                    return Err(());
                }
            };
        ...
        for seg in segments.iter().skip(1) {
            ...
            let seg_span = Span {
                ...
                end_off: path_span.start_off + seg.relative_start + seg.node.text_len().0,
            };
            ...
            match &sym.kind {
                ...
                SymbolKind::File { source_id } => {
                    ...
                    if let Some((found_sym, found_scope)) = self.scope_pool.lookup(file_scope, seg_name) {
                        ...
                        self.diag_collector.borrow_mut().add_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::NamePassError(
                                NamePassErrorKind::UndefinedName(seg_name.clone()),
                            )),
                            seg_span,
                            LocalizedMessage::new(MsgKind::NamePassUndefinedName, std::iter::once(seg_name.clone())),
                        );
                        return Err(());
                    }
                }
    ...
    fn build_expr_scope(
        ...
        match &expr.kind {
            ...
            GreenExprKind::Let { name, expr: e, .. } => {
                let name_span = child_span(&expr_red.span, name.relative_start, name.node.text_len().0);
            ...
            GreenExprKind::Match { for_match, arms } => {
                ...
                for arm_child in arms {
                    ...
                    let arm_span = child_span(&expr_red.span, arm_child.relative_start, arm.text_len.0);
                    ...
                    let pattern_abs_span = child_span(&arm_span, pattern_child.relative_start, pattern_child.node.text_len().0);
            ...
            GreenExprKind::Is { expr: e, pattern } => {
                ...
                let pattern_abs_span = child_span(&expr_red.span, pattern.relative_start, pattern.node.text_len().0);
                for (name, span) in bindings {
            ...
            GreenExprKind::With { handler_expr, clauses } => {
                ...
                for clause_child in clauses {
                    ...
                    let clause_span = child_span(&expr_red.span, clause_child.relative_start, clause.text_len.0);
                    ...
                    for param_child in &clause.params {
                        match &*param_child.node {
                            GreenCatchParam::Binding { name } => {
                                let param_span = child_span(&clause_span, param_child.relative_start, param_child.node.text_len().0);
                                self.scope_pool.add_symbol(
                                    catch_scope,
                                    name.name.clone(),
                                    param_span,
    ...
    fn resolve_expr(
        ...
        match &expr.kind {
            ...
            GreenExprKind::StaticPath { path } => {
                let path_span = child_span(&expr_red.span, path.relative_start, path.node.text_len.0);
                self.resolve_static_path_with_span(&path.node, &path_span, current_scope)?;
            }
            GreenExprKind::MemberAccess { left, .. } => {
                self.resolve_expr(&child_expr_red(&expr_red.span, left), current_scope)?;
            ...
            GreenExprKind::Raise { effect_path, control_name, args } => {
                let path_span = child_span(&expr_red.span, effect_path.relative_start, effect_path.node.text_len.0);
            ...
            GreenExprKind::With { handler_expr, clauses } => {
                ...
                for clause_child in clauses {
                    ...
                    let path_span = child_span(&clause_span, control_path_child.relative_start, control_path_child.node.text_len.0);
    ...
    fn resolve_name(
        ...
        if self.scope_pool.lookup(current_scope, name).is_some() {
            ...
            self.diag_collector.borrow_mut().add_error(
                ErrorKind::CompileTimeError(CompileTimeErrorKind::NamePassError(
                    NamePassErrorKind::UndefinedName(name.to_string()),
                )),
                span,
                LocalizedMessage::new(MsgKind::NamePassUndefinedName,
    ...
    fn build_scope(&mut self) -> Result<(), ()> {
        ...
        for file_unit in &self.ast_module.file_units {
                ...
                match &decl.kind {
                    GreenDeclKind::Fun { params, generic_vars, block, where_clause, .. } => {
                        ...
                        for gv in generic_vars {
                            let gv_span = child_span(&decl_span, gv.relative_start, gv.node.text_len.0);
                            self.scope_pool.add_symbol(
                                fun_scope_id,
                        ...
                        for param_child in params {
                            let param_span = child_span(&decl_span, param_child.relative_start, param_child.node.text_len.0);
                    ...
                    GreenDeclKind::TypeStruct { fields, generic_vars, has_abst, where_clause } => {
                        ...
                        for field_child in fields {
                            let field_span = child_span(&decl_span, field_child.relative_start, field_child.node.text_len.0);
                    ...
                    GreenDeclKind::ADT { ctors, generic_vars, has_abst, where_clause } => {
                        ...
                        for ctor_child in ctors {
                            let ctor_span = child_span(&decl_span, ctor_child.relative_start, ctor_child.node.text_len.0);
                            ...
                            for gv in &ctor_child.node.generic_vars {
                                let gv_span = child_span(&ctor_span, gv.relative_start, gv.node.text_len.0);
                                self.scope_pool.add_symbol(
                                    ctor_scope_id,
                                    gv.node.name.node.as_ref().name.clone(),
                                    gv_span,
                    ...
                    GreenDeclKind::Abstract { super_abst, generic_vars, methods, where_clause } => {
                        ...
                        for method_child in methods {
                            let method_span = child_span(&decl_span, method_child.relative_start, method_child.node.text_len.0);
                    ...
                    GreenDeclKind::Effect { controls } => {
                        ...
                        for ctrl_child in controls {
                            let ctrl_span = child_span(&decl_span, ctrl_child.relative_start, ctrl_child.node.text_len.0);
            ...
            for req_child in &file_unit.green.file_unit_requires {
                let req_red = {
                    ...
                    let len = req_child.node.text_len.0;
                    RequireRedNode {
                        span: Span {
    ...
    fn pass(mut self) -> (ResolvedNames, DiagCollector) {
        self.build_scope();
        self.resolve();
        (ResolvedNames {
