...
use leaf_coreapi::diagnose::{CompileTimeErrorKind, DiagCollector, DiagCtx, ErrorKind, LocalizedMessage, MirMonoErrorKind, MsgKind};
use leaf_coreapi::id::HirDeclId;
use leaf_coreapi::lang_items::BuiltinType;
use leaf_coreapi::mir::*;
use leaf_coreapi::type_ctx::{get_type_root, TyId, TypeNode};
use leaf_coreapi::type_ctx::{GenericParamDef, TypeCtx, TypeDef, TypeDefKind, TypeNodeKind};
...
impl MirMono {
    ...
    fn rvalue_ty(&self, rv: &Rvalue, fun: &MirFun) -> TyId {
        match rv {
            Rvalue::Move(Place::Local(id)) | Rvalue::Copy(Place::Local(id)) => {
                let raw_ty = fun.local_decls[id.0].ty;
    ...
    fn clone_fun_with_blocks(
        ...
        old_to_new_block: &mut HashMap<MirBasicBlockId, MirBasicBlockId>,
        global_map: &mut HashMap<MirBasicBlockId, MirBasicBlockId>,
    ) -> MirFun {
        let mut new_fun = fun.clone();
        for &old_bid in &fun.blocks {
            let new_bid = new_blocks.len();
            let mut new_block = old_blocks[old_bid.0].clone();
            new_blocks.push(new_block);
            old_to_new_block.insert(old_bid, MirBasicBlockId(new_bid));
            global_map.insert(old_bid, MirBasicBlockId(new_bid)); // 记录全局映射
        }
        for &old_bid in &fun.blocks {
            let new_bid = old_to_new_block[&old_bid];
            let block = &mut new_blocks[new_bid.0];
    ...
    fn remap_block_terminator(
        ...
        old_to_new: &HashMap<MirBasicBlockId, MirBasicBlockId>,
    ) {
        let map = |id: &mut MirBasicBlockId| { *id = old_to_new[id]; };
    ...
    fn mono(mut self) -> ((MirCrate, TypeCtx), DiagCollector) {
        let functions = std::mem::take(&mut self.mir.functions);
        let old_blocks = std::mem::take(&mut self.mir.blocks);
        let mut block_to_fun: HashMap<MirBasicBlockId, &MirFun> = HashMap::new();
        for fun in &functions {
            for &bid in &fun.blocks { block_to_fun.insert(bid, fun); }
        }

        let mut generic_calls: Vec<(MirBasicBlockId, MirFunId, Vec<TyId>)> = Vec::new();
        let mut inst_map: HashMap<MirFunId, Vec<Vec<TyId>>> = HashMap::new();

        for (bid, block) in old_blocks.iter().enumerate() {
            if let TerminatorKind::Call { func, args, dest, .. } = &block.terminator {
                let callee = &functions[func.0];
                if !callee.generic_params.is_empty() {
                    let caller_fun = block_to_fun[&MirBasicBlockId(bid)];
                    ...
                    for (rv, &param_ty) in args.iter().zip(callee.signature.params.iter()) {
                        ...
                        if !self.match_ty(param_ty, arg_ty, &mut mapping) {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::MirMonoError(MirMonoErrorKind::Generic)),
                                block.span.clone(),
                                LocalizedMessage::new(MsgKind::MirMonoGeneric, [format!("argument type mismatch in call to {}", callee.name)]),
                            );
                            //return Err(());
                            todo!()
                        }
                    ...
                    if let Place::Local(dest_id) = dest {
                        let ret_ty = get_type_root(self.get_type_pool(), caller_fun.local_decls[dest_id.0].ty);
                        if !self.match_ty(callee.signature.return_ty, ret_ty, &mut mapping) {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::MirMonoError(MirMonoErrorKind::Generic)),
                                block.span.clone(),
                                LocalizedMessage::new(MsgKind::MirMonoGeneric, [format!("return type mismatch in call to {}", callee.name)]),
                            );
                            //return Err(());
                            todo!()
                        }
                    ...
                    generic_calls.push((MirBasicBlockId(bid), *func, concrete_tys.clone()));
        ...
        let mut old_to_new_fun: HashMap<MirFunId, MirFunId> = HashMap::new();
        let mut old_to_new_instances: HashMap<MirFunId, HashMap<Vec<TyId>, MirFunId>> = HashMap::new();
        let mut global_block_map: HashMap<MirBasicBlockId, MirBasicBlockId> = HashMap::new();
        ...
        for (old_fid, fun) in functions.iter().enumerate() {
            ...
            old_to_new_fun.insert(MirFunId(old_fid), MirFunId(new_fid));
        }

        for (old_fid, fun) in functions.iter().enumerate() {
            if fun.generic_params.is_empty() { continue; }
            let instances = inst_map.get(&MirFunId(old_fid)).cloned().unwrap_or_default();
            let mut instance_map: HashMap<Vec<TyId>, MirFunId> = HashMap::new();
            for concrete_tys in &instances {
                ...
                let new_fid = new_functions.len();
                new_functions.push(new_fun);
                instance_map.insert(concrete_tys.clone(), MirFunId(new_fid));
            }
            old_to_new_instances.insert(MirFunId(old_fid), instance_map);
        }
        ...
        for (old_bid, fun_id, concrete_tys) in &generic_calls {
            ...
            if let Some(instance_map) = old_to_new_instances.get(fun_id) {
                ...
                if let TerminatorKind::Call { func, .. } = &mut new_blocks[new_bid.0].terminator {
                    *func = *new_func;
                }
        ...
        ((self.mir, self.type_checker_result), self.diag_collector.into_inner())
    }
}