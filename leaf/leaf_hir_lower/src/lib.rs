...
use leaf_coreapi::ast::{child_decl_red, child_expr_red, child_span_of, AtomExprNode, CrateAst, DeclRedNode, ExprRedNode, GreenCatchClause, GreenCatchParam, GreenChild, GreenCtor, GreenDecl, GreenDeclKind, GreenExpr, GreenExprKind, GreenField, GreenGenericVar, GreenMatchArm, GreenMethodDecl, GreenParam, GreenPattern, GreenPureStaticPath, GreenWhereClause, HasTextLen, TextLen, TypeName, Visibility};
use leaf_coreapi::diagnose::{CompileTimeErrorKind, DiagCtx, ErrorKind, HirLowerErrorKind, MsgKind, LocalizedMessage, DiagCollector};
use leaf_coreapi::hir::{HirBinOp, HirCatchClause, HirCatchParam, HirCrate, HirCtorDef, HirDecl, HirDeclKind, HirExpr, HirExprKind, HirFieldDef, HirGenericParam, HirLit, HirMatchArm, HirMethodDecl, HirName, HirParam, HirPattern, HirStructPatternField, HirTypeName, HirUnaryOp};
use leaf_coreapi::scope::{ResolvedNames, SymbolKind};
use leaf_coreapi::operators::Operator;
use leaf_coreapi::source::Span;
use std::sync::Arc;
use leaf_coreapi::id::{CrateId, HirDeclId, HirExprId, ScopeId, SymId};
...
impl<'a> HirLower<'a> {
    fn lookup_symbol(&self, scope_id: ScopeId, name: &str) -> Option<&leaf_coreapi::scope::Symbol> {
    ...
    fn resolve_static_path(
        ...
        if segments.is_empty() {
            self.diag_collector.borrow_mut().add_error(
                ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                    HirLowerErrorKind::EmptyPath,
                )),
                span.clone(),
                LocalizedMessage::new(MsgKind::HirLowerEmptyPath, std::iter::empty::<&str>()),
            );
            return Err(());
        }
        ...
        let (mut sym, mut current_scope) = match self
            .name_pass_result
            .pool
            .lookup(scope_id, first_name) {
                ...
                None => {
                    self.diag_collector.borrow_mut().add_error(
                        ...
                        first_span.clone(),
                        LocalizedMessage::new(MsgKind::HirLowerPathNotFound, std::iter::empty::<&str>()),
                    );
                    return Err(());
                }
            };
        ...
        for seg in segments.iter().skip(1) {
            ...
            match &sym.kind {
                SymbolKind::File { source_id } => {
                    let file_scope = match self.name_pass_result.source_id_to_scope.get(source_id) {
                        Some(s) => s,
                        None => {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                                    HirLowerErrorKind::ModuleScopeNotFound,
                                )),
                                seg_span.clone(),
                                LocalizedMessage::new(MsgKind::HirLowerModuleScopeNotFound, std::iter::empty::<&str>()),
                            );
                            return Err(());
                        }
                    };
                    let (found_sym, found_scope) = match self.name_pass_result.pool
                    ...
                    let field_sym = match fields
                        ...
                        .find(|s| s.name == *seg_name) {
                            ...
                            None => {
                                self.diag_collector.borrow_mut().add_error(
                                    ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                                        HirLowerErrorKind::FieldNotFound(seg_name.to_string()),
                                    )),
                                    seg_span.clone(),
                                    LocalizedMessage::new(MsgKind::HirLowerFieldNotFound, [seg_name.to_string()]),
                                );
                                return Err(());
                            }
                        };
                ...
                SymbolKind::ADT { constructors } => {
                    let ctor_sym = match constructors
                        .iter()
                        .filter_map(|&sid| self.name_pass_result.pool.get_symbol_by_id(sid))
                        .find(|s| s.name == *seg_name) {
                            Some(sym) => sym,
                            None => {
                                self.diag_collector.borrow_mut().add_error(
                                    ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                                        HirLowerErrorKind::ConstructorNotFound(seg_name.to_string()),
                                    )),
                                    seg_span.clone(),
                                    LocalizedMessage::new(MsgKind::HirLowerConstructorNotFound, [seg_name.to_string()]),
                                );
                                return Err(());
                            }
                        };
                    ...
                    let (found_sym, found_scope) = match self.name_pass_result.pool
                        .lookup(effect_scope, seg_name) {
                            ...
                            None => {
                                self.diag_collector.borrow_mut().add_error(
                                    ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                                        HirLowerErrorKind::ControlNotFound(seg_name.to_string()),
                                    )),
                                    seg_span.clone(),
                                    LocalizedMessage::new(MsgKind::HirLowerControlNotFound, [seg_name.to_string()]),
                                );
                                return Err(());
                            }
                        };
                ...
                SymbolKind::Generic => {
                    self.diag_collector.borrow_mut().add_error(
                        ...
                        seg_span.clone(),
                        LocalizedMessage::new(MsgKind::HirLowerInvalidPath, std::iter::empty::<&str>()),
                    );
                    return Err(());
                }
                _ => {
        ...
        match type_name {
            ...
            TypeName::Typeof { expr, .. } => {
                let expr_span = Span {
                    ...
                    end_off: span.start_off + expr.relative_start + expr.node.text_len.0,
                };
                ...
                let sym = match self
                    .lookup_symbol(scope_id, &gv.name.node.name) {
                        ...
                        None => {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                                    HirLowerErrorKind::GenericNotFound(gv.name.node.name.to_string()),
                                )),
                                gv_span.clone(),
                                LocalizedMessage::new(MsgKind::HirLowerGenericNotFound, [gv.name.node.name.to_string()]),
                            );
                            return Err(());
                        }
                    };
                ...
                let sym = match self
                    .lookup_symbol(scope_id, &ident.name) {
                        ...
                        None => {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                                    HirLowerErrorKind::BindingNotFound(ident.name.to_string()),
                                )),
                                span.clone(),
                                LocalizedMessage::new(MsgKind::HirLowerBindingNotFound, [ident.name.to_string()]),
                            );
                            return Err(());
                        }
                    };
            ...
            GreenPattern::Struct { path, fields, rest, .. } => {
                ...
                let hir_fields = fields
                    ...
                    .map(|f| {
                        ...
                        let field_name_sym = match self
                            .lookup_symbol(scope_id, &f.field_name.name) {
                                Some(sym) => sym,
                                None => {
                                    self.diag_collector.borrow_mut().add_error(
                                        ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                                            HirLowerErrorKind::FieldNotFound(f.field_name.name.to_string()),
                                        )),
                                        field_pat_span.clone(),
                                        LocalizedMessage::new(MsgKind::HirLowerFieldNotFound, [f.field_name.name.to_string()]),
                                    );
                                    return Err(());
                                }
                            };
                ...
                let sym = match self
                    .lookup_symbol(scope_id, &name.name) {
                        ...
                        None => {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                                    HirLowerErrorKind::BindingNotFound(name.name.to_string()),
                                )),
                                span.clone(),
                                LocalizedMessage::new(MsgKind::HirLowerBindingNotFound, [name.name.to_string()]),
                            );
                            return Err(());
                        }
                    };
        ...
        let sym = match self
            .lookup_symbol(scope_id, &green_field.name.node.name) {
                ...
                None => {
                    self.diag_collector.borrow_mut().add_error(
                        ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                            HirLowerErrorKind::FieldNotFound(green_field.name.node.name.to_string()),
                        )),
                        field_span.clone(),
                        LocalizedMessage::new(MsgKind::HirLowerFieldNotFound, [green_field.name.node.name.to_string()]),
                    );
                    return Err(());
                }
            };
        ...
        let sym = match self
            .lookup_symbol(scope_id, &green_param.name.node.name) {
                ...
                None => {
                    self.diag_collector.borrow_mut().add_error(
                        ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                            HirLowerErrorKind::ParamNotFound(green_param.name.node.name.to_string()),
                        )),
                        param_span.clone(),
                        LocalizedMessage::new(MsgKind::HirLowerParamNotFound, [green_param.name.node.name.to_string()]),
                    );
                    return Err(());
                }
            };
        ...
        let sym = match self
            .lookup_symbol(scope_id, &method_name.name) {
                ...
                None => {
                    self.diag_collector.borrow_mut().add_error(
                        ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                            HirLowerErrorKind::MethodNotFound(method_name.name.to_string()),
                        )),
                        method_span.clone(),
                        LocalizedMessage::new(MsgKind::HirLowerMethodNotFound, [method_name.name.to_string()]),
                    );
                    return Err(());
                }
            };
        ...
        let sym = match self
            .lookup_symbol(scope_id, &ctor_name.name) {
                ...
                None => {
                    self.diag_collector.borrow_mut().add_error(
                        ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                            HirLowerErrorKind::CtorNotFound(ctor_name.name.to_string()),
                        )),
                        ctor_span.clone(),
                        LocalizedMessage::new(MsgKind::HirLowerCtorNotFound, [ctor_name.name.to_string()]),
                    );
                    return Err(());
                }
            };
    ...
    fn lower_match_arm(
        ...
        let pattern_span = Span {
            ...
            end_off: parent_span.start_off + arm.pattern.node.text_len().0,
        };
        ...
        let arm_span = Span {
            source_id: parent_span.source_id,
            start_off: parent_span.start_off,
            end_off: parent_span.start_off + arm.text_len.0,
        };
        Ok(HirMatchArm {
            pattern,
            guard,
    ...
    fn lower_catch_clause(
            ...
            .map(|p| {
                ...
                match &*p.node {
                    GreenCatchParam::Binding { name } => {
                        let sym = match self.lookup_symbol(catch_scope, &name.name) {
                            Some(sym) => sym,
                            None => {
                                self.diag_collector.borrow_mut().add_error(
                                    ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
        ...
        let clause_span = Span {
            ...
            end_off: parent_span.start_off + clause.control_static_path.relative_start + clause.text_len.0,
        };
        Ok(HirCatchClause {
    ...
    fn lower_expr(
        ...
        let kind = match &expr_red.inner.kind {
                    ...
                    let sym = match self
                        .lookup_symbol(scope_id, name) {
                            ...
                            None => {
                                self.diag_collector.borrow_mut().add_error(
                                    ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                                        HirLowerErrorKind::NameNotFound(name.to_string()),
                                    )),
                                    span.clone(),
                                    LocalizedMessage::new(MsgKind::HirLowerNameNotFound, [name.to_string()]),
                                );
                                return Err(());
                            }
                        };
            ...
            GreenExprKind::Call { callee, args } => {
                ...
                let param_names = if has_named {
                    match maybe_func_sym.as_ref()
                        .and_then(|hir_name| self.param_names_map.get(&hir_name.sym_id)) {
                            Some(pn) => pn,
                            None => {
                                self.diag_collector.borrow_mut().add_error(
                                    ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                                        HirLowerErrorKind::CannotResolveFunction,
                                    )),
                                    span.clone(),
                                    LocalizedMessage::new(MsgKind::HirLowerCannotResolveFunction, std::iter::empty::<&str>()),
                                );
                                return Err(());
                            }
                        }
                ...
                let ordered_args = if has_named {
                    ...
                    for arg in args {
                        if let Some(name_child) = &arg.node.name {
                            ...
                            if !seen_names.insert(name.clone()) {
                                self.diag_collector.borrow_mut().add_error(
                                    ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                                        HirLowerErrorKind::DuplicateKeywordArg,
                                    )),
                                    span.clone(),
                                    LocalizedMessage::new(MsgKind::HirLowerDuplicateKeywordArg, std::iter::empty::<&str>()),
                                );
                                return Err(());
                            }
                    ...
                    for name in named.keys() {
                        if !param_names.contains(name) {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                                    HirLowerErrorKind::UnexpectedKeywordArg,
                                )),
                                span.clone(),
                                LocalizedMessage::new(MsgKind::HirLowerUnexpectedKeywordArg, std::iter::empty::<&str>()),
                            );
                            return Err(());
                        }
                    }
                    ...
                    if positional.len() + named.len() > total_params {
                        self.diag_collector.borrow_mut().add_error(
                            ...
                            span.clone(),
                            LocalizedMessage::new(MsgKind::HirLowerTooManyArguments, std::iter::empty::<&str>()),
                        );
                        return Err(());
                    }
                    ...
                    for (name, arg) in named {
                        ...
                        if ordered[idx].is_some() {
                            self.diag_collector.borrow_mut().add_error(
                                ...
                                span.clone(),
                                LocalizedMessage::new(MsgKind::HirLowerArgumentConflict, std::iter::empty::<&str>()),
                            );
                            return Err(());
                        }
                    ...
                    if ordered.iter().any(|o| o.is_none()) {
                        self.diag_collector.borrow_mut().add_error(
                            ...
                            span.clone(),
                            LocalizedMessage::new(MsgKind::HirLowerMissingArguments, std::iter::empty::<&str>()),
                        );
                        return Err(());
                    }
                    ...
                    if let Some(sym) = self.name_pass_result.pool.get_symbol_by_id(func_sym.sym_id) {
                        if let SymbolKind::Constructor = sym.kind {
                            if ordered_args.len() == 1 {
                                ...
                                self.hir.hir_expr_pool.push(
                                    HirExpr { 
                                        kind, 
                                        hir_id: HirExprId(hir_id), 
                                        span 
                                    });
                                return Ok(HirExprId(hir_id));
                            } else {
                                self.diag_collector.borrow_mut().add_error(
                                    ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                                    ...
                                    span.clone(),
                                    LocalizedMessage::new(MsgKind::HirLowerArityMismatch, std::iter::empty::<&str>()),
                                );
                                return Err(());
                            }
                        }
            ...
            GreenExprKind::StaticPath { path } => {
                    ...
                    if let Some(sym) = self.name_pass_result.pool.get_symbol_by_id(path_name.sym_id) {
                        if let SymbolKind::Constructor = sym.kind {
                            ...
                            self.hir.hir_expr_pool.push(HirExpr {
                                ...
                                hir_id: HirExprId(unit_id),
                                span: span.clone(),
                            });
                            let kind = HirExprKind::BuildVariant {
                                variant_name: path_name,
                                target: HirExprId(unit_id),
                            };
                            let hir_id = self.hir.hir_expr_pool.len();
                            self.hir.hir_expr_pool.push(HirExpr { kind, hir_id: HirExprId(hir_id), span: span.clone() });
                            return Ok(HirExprId(hir_id));
                        }
                    }
                }
                ...
                for seg in segments.iter().skip(start_field_idx) {
                    ...
                    let field_hir_id = HirExprId(self.hir.hir_expr_pool.len());
                ...
                let sym = match self
                    .lookup_symbol(scope_id, &name.node.name) {
                        ...
                        None => {
                            self.diag_collector.borrow_mut().add_error(
                                ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                                    HirLowerErrorKind::LetNameNotFound(name.node.name.to_string()),
                                )),
                                span.clone(),
                                LocalizedMessage::new(MsgKind::HirLowerLetNameNotFound, [name.node.name.to_string()]),
                            );
                            return Err(());
                        }
                    };
                ...
                let hir_arms = arms
                    ...
                    .map(|arm_child| {
                        ...
                        let arm_span = Span {
                            ...
                            end_off: span.start_off + arm_child.relative_start + arm.text_len.0,
                        };
                ...
                let hir_clauses = clauses
                    ...
                    .map(|clause_child| {
                        ...
                        let clause_span = Span {
                            ...
                            end_off: span.start_off + clause_child.relative_start + clause.text_len.0,
                        };
            ...
            GreenExprKind::PipeLine { left, right } => {
                ...
                let (callee_id, mut args) = match &right_red.inner.kind {
                    ...
                    _ => {
                        self.diag_collector.borrow_mut().add_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                                HirLowerErrorKind::InvalidPipeLineTarget,
                            )),
                            span.clone(),
                            LocalizedMessage::new(MsgKind::HirLowerInvalidPipeLineTarget, std::iter::empty::<&str>()),
                        );
                        return Err(());
                    }
                };
        ...
        let hir_id = self.hir.hir_expr_pool.len();
        let hir_expr = HirExpr { kind, hir_id: HirExprId(hir_id), span };
            ...
            GreenDeclKind::Effect { controls } => {
                let controls_hir = controls
                    ...
                    .map(|ctrl| {
                        ...
                        let sym = match self.lookup_symbol(decl_scope, &ctrl_name_str) {
                            Some(sym) => sym,
                            None => {
                                self.diag_collector.borrow_mut().add_error(
                                    ErrorKind::CompileTimeError(CompileTimeErrorKind::HirLowerError(
                                        HirLowerErrorKind::ControlNotFound(ctrl_name_str.to_string()),
                                    )),
                                    ctrl_span.clone(),
                                    LocalizedMessage::new(MsgKind::HirLowerControlNotFound, [ctrl_name_str.to_string()]),
                                );
                                return Err(());
                            }
                        };
    ...
    fn convert_unary(op: &Operator) -> Result<HirUnaryOp, ()> {
        match op {
    ...
    fn lower(mut self) -> (HirCrate, DiagCollector) {
        ...
        (self.hir, self.diag_collector.into_inner())
    }
}
...
fn hir_lower(
    ...
    module_name: String,
    crate_id: CrateId
) -> (HirCrate, DiagCollector) {
    let lower = HirLower::new(ast_crate, name_pass_result, module_name, crate_id);
    lower.lower()
}