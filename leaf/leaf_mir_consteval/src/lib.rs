use leaf_coreapi::diagnose::{CompileTimeErrorKind, DiagCollector, ErrorKind, LocalizedMessage, MirConstEvalErrorKind, MsgKind};
...
impl MirConstEval {
    ...
    fn value_to_const(&self, value: &Value, ty: TyId, span: Span) -> Result<Const, ()> {
        ...
        match (value, kind) {
            ...
            (Value::Tuple(elems), TypeNodeKind::Tuple(tys)) => {
                ...
                let consts: Result<Vec<_>, _> = elems.iter().zip(tys).map(
                    |(v, ty)|
                        self.value_to_const(v, *ty, span.clone()
                )).collect();
            ...
            (Value::Enum(tag, data), TypeNodeKind::ADT { variants, .. }) => {
                let payload_ty = match variants.get(tag.0).copied().flatten() {
                    Some(v) => v,
                    None => {
                        self.diag_collector.borrow_mut().add_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::MirConstEvalError(MirConstEvalErrorKind::EvalFailed)),
                            span.clone(),
                            LocalizedMessage::new(MsgKind::MirConstEvalFailed, [format!("invalid enum variant tag {}", tag.0)]),
                        );
                        return Err(());
                    }
                };
    ...
    fn write_place(&self, place: &Place, value: Value, frame: &mut Frame, span: Span) -> Result<(), ()> {
        match place {
            Place::Local(id) => {
                frame.locals[id.0] = value;
                Ok(())
            }
    ...
    fn eval_place_to_value(&self, place: &Place, frame: &Frame, span: Span) -> Result<Value, ()> {
        match place {
            Place::Local(id) => match frame.locals.get(id.0).cloned() {
                Some(v) => Ok(v),
                None => {
                    self.diag_collector.borrow_mut().add_error(
                        ErrorKind::CompileTimeError(CompileTimeErrorKind::MirConstEvalError(MirConstEvalErrorKind::EvalFailed)),
                        span,
                        LocalizedMessage::new(MsgKind::MirConstEvalFailed, [format!("local {} not initialized", id.0)]),
                    );
                    Err(())
                }
            },
                ...
                match cache.get(sid).map(|c| Self::const_to_value(c)) {
                    ...
                    None => {
                        self.diag_collector.borrow_mut().add_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::MirConstEvalError(MirConstEvalErrorKind::EvalFailed)),
                            span,
                            LocalizedMessage::new(MsgKind::MirConstEvalFailed, [format!("static {} not evaluated", sid.0)]),
                        );
                        Err(())
                    }
                }
    ...
    fn eval_rvalue(&self, rvalue: &Rvalue, frame: &Frame, span: Span) -> Result<Value, ()> {
        match rvalue {
            ...
            Rvalue::HandlerArg(idx) => {
                ...
                match frame.locals.get(local_id.0).cloned() {
                    Some(v) => Ok(v),
                    None => {
                        self.diag_collector.borrow_mut().add_error(
                            ErrorKind::CompileTimeError(CompileTimeErrorKind::MirConstEvalError(MirConstEvalErrorKind::EvalFailed)),
    ...
    fn cast_value(&self, value: Value, target_ty: TyId, span: Span) -> Result<Value, ()> {
    ...
    fn eval_binary(&self, op: &MirBinOp, l: Value, r: Value, span: Span) -> Result<Value, ()> {
        use MirBinOp::*;
        match op {
            Add => {
    ...
    fn eval_unary(&self, op: &MirUnOp, v: Value, span: Span) -> Result<Value, ()> {
        match op {
            MirUnOp::Neg => match v {
    ...
    fn push_context(
        ...
        let fun = self.mir.functions[fun_id.0].clone();
    ...
    fn handle_raise(
        ...
        while self.context_stack.len() > handler_ctx_idx + 1 {
            ...
            self.cleanup_handlers_for_ctx(ctx.fun_id.0);
        ...
        let handler_block_obj = &self.mir.blocks[handler_entry.handler_block.0];
        {
            ...
            for (i, &param_local) in handler_block_obj.block_params.iter().enumerate() {
                ...
                handler_frame.locals[param_local.0] = val;
            }
            for (i, &local_id) in handler_entry.args_dest.iter().enumerate() {
                let val = raised_vals.get(i).cloned().unwrap_or(Value::Unit);
                handler_frame.locals[local_id.0] = val;
            }
        }
    ...
    fn run_stack(&mut self) -> Result<Value, ()> {
        loop {
...

            {
                ...
                let block = &self.mir.blocks[block_id.0];
                ...
                terminator = {
                    let block_id = self.context_stack[ctx_idx].current_block;
                    self.mir.blocks[block_id.0].terminator.clone()
                };
            }
            ...
            match &terminator {
                TerminatorKind::Return => {
                    ...
                    if let Some(dest) = &ret_dest {
                        match dest {
                            Place::Local(local) => {
                                ...
                                caller_frame.locals[local.0] = ret.clone();
                            }
                            _ => {
                                self.diag_collector.borrow_mut().add_error(
                                    ErrorKind::CompileTimeError(CompileTimeErrorKind::MirConstEvalError(MirConstEvalErrorKind::EvalFailed)),
                    ...
                    if let (Some(call_block), Some(dest), Some(target)) = (call_block_id, &ret_dest, ret_target) {
                        ...
                        if let Place::Local(local) = dest {
                            let block = &mut self.mir.blocks[call_block.0];
                ...
                TerminatorKind::Goto { target, block_args } => {
                    if !block_args.is_empty() {
                        let target_block = &self.mir.blocks[target.0];
                        if let Some(&first_param) = target_block.block_params.first() {
                            ...
                            drop(frame_ref);
                            let mut frame_mut = self.context_stack[ctx_idx].frame.borrow_mut();
                            frame_mut.locals[first_param.0] = val;
                        }
                    }
                ...
                TerminatorKind::SwitchInt { discriminant, targets, default } => {
                    ...
                    if let Value::Enum(tag, _) = disc_val {
                        disc_val = Value::Int(tag.0 as i64);
                    }
                ...
                TerminatorKind::Call { func, args, dest, target } => {
                    let callee_id = *func;
                    let callee = &self.mir.functions[callee_id.0];
                ...
                TerminatorKind::InstallHandler { handler_block, next, args_dest, control_id } => {
                    let handler_term = &self.mir.blocks[handler_block.0].terminator;
    ...
    fn build_local_const_map(
        fun: &MirFun, blocks: &[BasicBlock]
    ) -> HashMap<MirLocalId, Const> {
    ...
    fn reachable_functions(&self, main_id: MirFunId) -> HashSet<MirFunId> {
        ...
        while let Some(fid) = queue.pop_front() {
            let fun = &self.mir.functions[fid.0];
            for &block_id in &fun.blocks {
                let block = &self.mir.blocks[block_id.0];
    ...
    fn eval(mut self) -> ((MirCrate, TypeCtx), DiagCollector) {
        {
            let mut cache = self.static_cache.borrow_mut();
            for (sid, s) in self.mir.statics.iter().enumerate() {
                cache.insert(MirStaticId(sid), s.init.clone());
            }
        }
        ...
        let reachable = self.reachable_functions(MirFunId(main_id));
        ...
        for &fid in &reachable {
            ...
            for &block_id in &fun.blocks {
                ...
                if let TerminatorKind::Call { func, args, dest, target } = &block.terminator {
                    let callee_id = func;
                    if !self.mir.functions[callee_id.0].is_consteval {
                        continue;
                    }
        ...
        for (caller_fun_id, block_id, callee_id, const_args, dest, target) in pending {
            ...
            if self.context_stack.is_empty() {
                let ret_ty = self.mir.functions[callee_id.0].signature.return_ty;
                let const_val = self.value_to_const(
                    &result, ret_ty, self.mir.functions[callee_id.0].span.clone())
                    .unwrap();
                if let Place::Local(local) = dest {
                    let block = &mut self.mir.blocks[block_id.0];
                    block.statements.push(MirStmt {
                        kind: MirStmtKind::Let {
                            local,
                            rvalue: Rvalue::Constant(const_val),
        ...
        for (sid, const_val) in cache.iter() {
            self.mir.statics[sid.0].init = const_val.clone();
        }
